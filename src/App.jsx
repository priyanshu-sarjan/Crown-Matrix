...
import {
  ...
  ChevronLeft, Award, Zap, Trophy, ShieldCheck, CheckCircle2,
  Calendar, Flame, Play, HelpCircle
} from 'lucide-react';
...
import { CAMPAIGN_LEVELS, getDailyPuzzle, REGION_COLORS } from './logic/levels';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col font-sans selection:bg-cyan-500 selection:text-slate-950">
      ...
      {/* Main Content Body */}
      <main className="flex-1 flex flex-col items-center justify-between p-4 sm:p-6 max-w-6xl w-full mx-auto">
        
        {/* Top Info Ribbon */}
        <div className="w-full flex flex-wrap items-center justify-between gap-4 bg-slate-900/60 p-4 rounded-2xl border border-slate-800 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4 text-xs font-semibold">
            ...
            {/* Timer */}
            {mode !== 'blitz' ? (
              <div className="bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800 font-mono text-cyan-400 text-sm">
                ⏱️ {formatTime(timer)}
              </div>
            ) : (
              <div className="bg-amber-950 text-amber-300 px-3 py-1.5 rounded-xl border border-amber-800 font-mono text-sm font-bold flex items-center gap-1.5 animate-pulse">
        ...
        {hint && (
          <div
            className={`w-full my-3 p-3.5 rounded-2xl border text-xs sm:text-sm font-semibold flex items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2 duration-200 ${
              hint.type === 'error'
        ...
        <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
          ...
          {mode === 'campaign' && (
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={levelIdx === 0}
                className="p-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white disabled:opacity-30 disabled:pointer-events-none transition-colors"
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold text-xs shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-600 text-slate-300 hover:text-white font-semibold text-xs disabled:opacity-30 transition-all"
            >
            ...
            <button
              onClick={handleReset}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 hover:border-red-500/50 text-slate-300 hover:text-red-300 font-semibold text-xs transition-all"
            >
        ...
        {threats.isSolved && (
          <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-300">
            <div className="p-8 rounded-3xl bg-slate-900 border-2 border-cyan-400/50 text-center max-w-md shadow-[0_0_80px_rgba(6,182,212,0.4)]">
              <div className="w-16 h-16 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/50 flex items-center justify-center mx-auto mb-4 animate-bounce">
                <Crown className="w-8 h-8" />
              </div>
              <h3 className="text-3xl font-black text-white mb-2">Matrix Conquered!</h3>
